...
#         return lambda idx: generate_vibration_data(idx)
#     elif simulator_type == "current":
#         return lambda idx: generate_current_data(idx)
#     else:
...
def simulate_data(count, interval, sensor_num=2,callback=None, simulator_type="humidity_temp"):
    try:
        ...
        # 데이터 생성 함수 선택
        simulator = get_simulator(simulator_type) # 함수 , 토픽을 반환
        topic_name = simulator.generate_data(0)["id"] # 첫 번째 센서의 ID를 사용하여 토픽 이름 생성
        for _ in range(count):
            for sensor_idx in range(sensor_num):
                # 선택된 데이터 생성 함수 호출
                payload = simulator(sensor_idx) # generate_temp_data(sensor_idx)와 같은 형태로 호출됨
