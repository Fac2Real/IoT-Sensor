...
class PressureSimulator(ContinuousSimulatorMixin,SimulatorInterface2):
    # 타입별 시뮬레이터 세팅
    SENSOR_TYPE  = "pressure"
    # 환경센서용 정규분포 파라미터
    ENV_MU, ENV_SIGMA    = 35.74, 10.38
    # 설비센서용 정규분포 파라미터  
    FAC_MU, FAC_SIGMA    = 35.74, 10.38  # 설비센서: 평균 45도, 표준편차 15 (변동성 큼)

    ENV_LOWER, ENV_UPPER = 3.5, 80
    
    FAC_LOWER, FAC_UPPER = 3.5, 80
    OUTLIER_P    = 0.05
    ...
    def __init__(self, idx: int, zone_id:str, equip_id:str, interval:int = 5, msg_count:int = 10, conn=None):
        ...
        #########################################
        self.sensor_id = f"UA10H-PRS-2406089{idx}" # 센서 ID
        self.type = "pressure" # 센서 타입
    ...
    def _apply_desired_state(self, desired_state):
        target_pressure = desired_state.get("target_pressure")
        if target_pressure is not None:
            self.target_pressure = target_pressure
            print(f"Desired state applied: {self.sensor_id} - Target pressure: {self.target_pressure}")
        else:
            print(f"No target pressure provided for {self.sensor_id}.")
